...
export default function LearningExperience() {
  return (
    <section className="section-padding bg-[#FFF8F0]" id="learning">
      <div className="max-w-7xl mx-auto">
        <motion.div className="text-center mb-14" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          <span className="text-[#FF9933] text-sm tracking-[0.2em] uppercase font-semibold">How It Works</span>
          <h2 className="font-heading text-3xl sm:text-4xl md:text-5xl text-gray-900 mt-3 font-bold">Your Learning Experience</h2>
        </motion.div>

        <div className="relative">
          {/* Connecting line */}
          <div className="hidden md:block absolute top-20 left-[10%] right-[10%] h-px">
            <motion.div className="h-full border-t-2 border-dashed border-[#FF9933]/20" initial={{ scaleX: 0 }} whileInView={{ scaleX: 1 }} viewport={{ once: true }} transition={{ duration: 1.2, delay: 0.3 }} />
          </div>
            ...
            {steps.map((s, i) => (
              ...
              >
                ...
                <div className="relative z-10 pt-6">
                  <div className="w-14 h-14 mx-auto rounded-2xl bg-white border border-[#FF9933]/15 flex items-center justify-center mb-4 shadow-sm">
                    <s.icon className="w-6 h-6 text-[#FF9933]" />
                  </div>
                  <h3 className="font-heading text-sm font-bold text-gray-900 mb-2">{s.title}</h3>
